...
- Added a limited acceleration model with user controls for acceleration factor R and a generic 0–1 encoding-diversity factor.
- Added live readouts for relative sampling burden, a synthetic g-like noise-amplification proxy, and an SNR-like efficiency proxy using `1 / (√R · g̃)`.
- Added a visualization of retained synthetic phase-line burden and two stylized encoding profiles that separate more strongly as encoding diversity increases.
- Kept g̃ explicitly non-physical: it is an invented teaching relationship rather than a measured g-factor or reconstruction output.
- The preview does not model real coil arrays, calibration data, alias unfolding, reconstruction algorithms, noise covariance, image quality, or protocol adequacy.
- Diffusion & b-Value remains an interactive premium teaser; RF Power Concepts remains a locked product preview.
- Billing remains disabled; v11.2 cannot initiate a charge.
- Updated visible release identity and offline cache marker to v11.2 / v11.2.0.

### v11.1 release notes
- Turned **Diffusion & b-Value Lab** into the first interactive Premium Lab teaser.
- Added a limited normalized mono-exponential diffusion model using `S/S₀ = e^(−bD)`.
- Added preview controls for b-value and a generic modeled diffusivity coefficient.
- Added live normalized-signal and attenuation readouts plus a plotted diffusion-decay curve.
- Kept the teaser intentionally generic: no tissue labels, lesion categories, diagnostic thresholds, or patient-specific ADC interpretation.
- The full paid Diffusion Lab remains reserved for authenticated premium access and is planned to add multi-model comparison, structured sweeps, saved experiments, and deeper teaching prompts.
- Parallel Imaging and RF Power Concepts remain locked product previews.
- Billing remains disabled; v11.1 cannot initiate a charge.
- Updated visible release identity and offline cache marker to v11.1 / v11.1.0.
...
- Added a visible **v10.2** build badge beside the MR Command Center brand so the running release can be identified immediately.
- Kept **Edon Kukaj** visible in the responsive brand treatment instead of hiding the creator credit on narrower screens.
- Added an always-visible footer signature: **Created by Edon Kukaj · v10.2**.
- Added a live current-route chip on larger screens for clearer workspace orientation.
- Added mobile safe-area handling for the bottom navigation.
- Added automatic centering of the active Lab button in the horizontally scrolling mobile Lab switcher.
- Corrected workspace restore copy so it refers to the current v10 active workspace rather than the older v8 wording.
